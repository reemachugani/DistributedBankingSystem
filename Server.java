...
public class Server {
    ...
    public static void sendDataPacket(DatagramSocket serverSocket, byte[] sendData, InetAddress IPAddress, int port)throws Exception{
//        Thread.sleep(10000);
...
//        accNum, curr, amt, curBal, transferAccNum
        ...
        for(Account obj : accArr){
            if(obj.accNumber == Integer.parseInt(elem[5].trim())){
                ...
                String sender = updateBalance(Integer.parseInt(elem[1].trim()), amt*-1);
...
//                msg = "6|"+ sender + "|" + elem[1] + "|" + elem[5] + "|" + amtDisp;
    ...
    public static void appendAccNum(int accNum)throws IOException{
        PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter("AccNum", true)));
        out.println(accNum);
        out.close();
    }

    public static void appendAccountObj(Account obj)throws IOException{
        PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter("AccountObjects", true)));
